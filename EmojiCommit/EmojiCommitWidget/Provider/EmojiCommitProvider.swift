...
class EmojiCommitProvider: TimelineProvider {
    ...
    func placeholder(in context: Context) -> EmojiCommitEntry {
        ...
        return EmojiCommitEntry(date: Date(), commits: defualtCommit)
    }
    ...
    func getSnapshot(in context: Context, completion: @escaping (EmojiCommitEntry) -> Void) {
        ...
        let defualtCommit = (0...6).map { (day) -> Commit in
            let nextDate = Calendar.current.date(byAdding: .day, value: day, to: defaultDate)
            let commit = Commit(date: nextDate ?? defaultDate, level: Commit.Level(rawValue: day % 5) ?? .zero)
            return commit
        }

        let entry = EmojiCommitEntry(date: Date(), commits: defualtCommit)
        completion(entry)
    }
    ...
    func getTimeline(in context: Context, completion: @escaping (Timeline<Entry>) -> Void) {
        ...
        GithubService(apiService: APIService(),
            ...
            .map {
                Timeline(entries: [EmojiCommitEntry(date: currentDate,
                                                    commits: $0)],
                         policy: .after(refreshDate))
            }
            .replaceError(with: Timeline(entries: [EmojiCommitEntry(date: currentDate,
                                                                    commits: [])],
                                         policy: .after(refreshDate)))
